...
(defun highlight-tail-post-command ()
  ...
  (when (and highlight-tail-mode
             (or (equal this-command 'self-insert-command)
                 ...
                 (equal this-command 'tab-to-tab-stop)
		 ;;(equal this-command 'forward-char-nomark)
		 ;;(equal this-command 'backward-char-nomark)
		 (equal this-command 'completion-separator-self-insert-autofilling)
		 (equal this-command 'completion-separator-self-insert-command)))
...
Run it, when you've made changes to some highlight-tail-mode variables."
  ...
  (if (equal window-system nil)
      (message
       "Highlight-tail disabled because not running under window system.")
